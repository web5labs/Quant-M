...
pub fn authority_records() -> Vec<FsmAuthorityRecord> {
    vec![
        ...
        FsmAuthorityRecord {
            ...
            gates_side_effects: true,
            known_limitations: vec![
                "wired for current runnable skills and selected high-risk runtime paths",
                "trading-like actions remain unavailable; no broker or exchange execution is implemented",
                "file writes and state mutations still use local path-specific validation rather than a universal gate",
            ],
        },
        FsmAuthorityRecord {
